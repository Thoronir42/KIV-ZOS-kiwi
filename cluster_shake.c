...
int shake_analyze_fat(struct shake_farmer *p_s_f) {
	...
	printf("FAT analyzation starting with chunk size %d\n", p_s_f->CLUSTER_CHUNK_SIZE);
	...
	if (non_empty % p_s_f->CLUSTER_CHUNK_SIZE) {
		...
		p_s_f->cluster_chunk_last_size = non_empty % p_s_f->CLUSTER_CHUNK_SIZE;
	} else {
...
void *shake_worker_run(struct shake_worker * p_s_w) {
	...
	int chunk_offset, search_chunk_start;
